...
const NavTools = ({
  ...
  return (
    <div className="nav-tools flex items-center  gap-2">
      ...
      <ThemeButton />
      <div className=" h-8 bg-[#282F5A] w-px"></div>
      <Inbox />
      <div className=" h-8 bg-[#282F5A] w-px"></div>
      <NotificationMessage />

      <div className={`${ttCommonsRegular.className} text-[#282F5A] `}>
        <p className="text-[16px] font-semibold">Flora</p>
        <p className="text-[10px] font-medium"> Admin</p>
      </div>
      <div className="ltr:pl-2 rtl:pr-2 flex items-center gap-x-1">
        <ProfileInfo />
        <MdKeyboardArrowDown />
      </div>
...
const Header = ({
  ...
  if (layout === "horizontal" && navbarType !== "hidden") {
    return (
      <ClassicHeader
        className={cn(" ", {
          "sticky top-0 z-0": navbarType === "sticky",
        })}
      >
  ...
  return (
    ...
    >
      <div className="w-[95%] bg-card/90 backdrop-blur-lg md:px-6 px-[15px] py-3 border-b z-0 absolute left-[60px] top-[30px] shadow-md rounded-[10px]">
